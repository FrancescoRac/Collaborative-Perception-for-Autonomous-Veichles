...
def box_to_corner(box: np.ndarray) -> np.ndarray:  # TODO
    """
    ...
    R = rot_z(box[6]) # Rotation matrix

    center = np.array(box[:3]) #@ R

    dx, dy, dz = box[3:6]

    corners = np.zeros((8, 3))# this is just a dummy value
    
    corners[0,:] = np.array([dx / 2  , dy / 2  , dz / 2])
    corners[1,:] = np.array([dx / 2  , -dy / 2 , dz / 2])
    corners[2,:] = np.array([dx / 2  , -dy / 2 , -dz / 2])
    corners[3,:] = np.array([dx / 2  , dy / 2  , -dz / 2])
    corners[4,:] = np.array([-dx / 2 , dy / 2  , dz / 2])
    corners[5,:] = np.array([-dx / 2 , -dy / 2 , dz / 2])
    corners[6,:] = np.array([-dx / 2 , -dy / 2 , -dz / 2])
    corners[7,:] = np.array([-dx / 2 , dy / 2  , -dz / 2])

    corners = np.dot(corners, R.transpose()) # Rotate the corners
    corners += center # Translate the corners
...
def box_to_pixels(boxes, bev_imsize, bev_resolution):
    ...
    # TODO: find the pixel coordinates of the corners for all boxes (DA VEDERE)
...
def points_to_pixels(filtered_points : np.ndarray, bev_imsize: np.ndarray , bev_resolution: float):
    ...
    '''
    # TODO: find pixel coordinates for the points in bev image (DA VEDERE)
